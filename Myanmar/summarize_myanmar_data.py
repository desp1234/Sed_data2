...
def summarize_nc_file(filepath):
    ...
    try:
        with nc.Dataset(filepath, 'r') as ds:
            # Get river name from global attributes
            river_name = ds.river_name if hasattr(ds, 'river_name') else 'Unknown'
            ...
            for var_name in ['Q', 'SSC', 'SSL']:
                if var_name in ds.variables and f"{var_name}_flag" in ds.variables:
                    ...
                    fill_value = ds.variables[var_name].missing_value if hasattr(ds.variables[var_name], 'missing_value') else -9999.0
                    ...
                    if good_data.size > 0:
                        ...
                        summary[var_name] = f"No 'good' data found (count: {good_data.size})"
                else:
