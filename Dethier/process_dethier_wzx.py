...
def apply_qc_flags_q(q):
    """
    对 Q 应用 QC flag
    """
    flag = np.full_like(q, FLAG_MISS, dtype=np.int8)
    mask_notnan = ~np.isnan(q)

    # bad: negative
    bad = mask_notnan & (q < 0)
    flag[bad] = FLAG_BAD

    # suspect: zero or extreme high
    zero = mask_notnan & (q == 0)
    extreme = mask_notnan & (q > Q_EXTREME_HIGH)
    flag[zero | extreme] = FLAG_SUSPECT

    # good: 非 NaN 且非 bad/suspect
    good = mask_notnan & ~(bad | zero | extreme)
    flag[good] = FLAG_GOOD

    return flag


def apply_qc_flags_ssc(ssc):
    """
    对 SSC 应用 QC flag
    """
    flag = np.full_like(ssc, FLAG_MISS, dtype=np.int8)
    mask_notnan = ~np.isnan(ssc)

    bad = mask_notnan & (ssc < 0)
    flag[bad] = FLAG_BAD

    suspect = mask_notnan & (ssc > SSC_EXTREME_HIGH)
    flag[suspect] = FLAG_SUSPECT

    good = mask_notnan & ~(bad | suspect)
    flag[good] = FLAG_GOOD

    return flag


def apply_qc_flags_ssl(ssl):
    """
    对 SSL 应用 QC flag
    """
    flag = np.full_like(ssl, FLAG_MISS, dtype=np.int8)
    mask_notnan = ~np.isnan(ssl)

    bad = mask_notnan & (ssl < 0)
    flag[bad] = FLAG_BAD

    extreme = mask_notnan & (ssl > SSL_EXTREME_HIGH)
    flag[extreme] = FLAG_SUSPECT

    good = mask_notnan & ~(bad | extreme)
    flag[good] = FLAG_GOOD
...
def process_single_netcdf(nc_path: str, output_dir: str) -> dict | None:
    ...
    q_flag = apply_qc_flags_q(q_arr)
    ssc_flag = apply_qc_flags_ssc(ssc_arr)
    ssl_flag = apply_qc_flags_ssl(ssl_arr)
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = "/share/home/dq134/wzx/sed_data/sediment_wzx_1111/Script/Dataset/Dethier/Output_r"
    SUMMARY_CSV = "/share/home/dq134/wzx/sed_data/sediment_wzx_1111/Script/Dataset/Dethier/Output_r/Dethier_station_summary.csv"
